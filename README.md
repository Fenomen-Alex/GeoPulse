...
1. Unauthenticated users see the landing page; calling `/api/v1/spatial/isochrone` without a session cookie returns `401`.
...
4. 16th spatial request returns HTTP `429` and triggers the Quota Extension Modal.
5. No upstream API keys (`ORS_API_KEY`, `RESEND_API_KEY`) appear in client bundle or network inspector.
